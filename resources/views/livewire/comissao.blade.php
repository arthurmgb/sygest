<div>
    <div class="div-block-area-3">
        ...
        <div class="admin-area">
            ...
            <div class="card">
                    ...
                    @if ($modalidade_comissao === 1)
                        ...
                        <div class="card-topo mr-2">
                            <div style="margin-bottom: 0 !important;" class="title-block f-calc">                                                           
                                Comissões à pagar
                                <span class="period">/ Quantidade: {{$get_count_geral_comissoes_a_pagar}}</span>
                            </div>                     
                        </div>
                        <button wire:key="btn3" wire:click.prevent="alternarModalidadeComissao(0)" wire:loading.attr="disabled" class="btn btn-sm btn-outline-success ml-2 p-1" data-tooltip="Comissões pagas" data-flow="top">
                        ...
                        @elseif($modalidade_comissao === 0)  
                            ...
                            <div class="card-topo mr-2">
                                <div style="margin-bottom: 0 !important;" class="title-block f-calc">                                                           
                                    Comissões pagas
                                    <span class="period">/ Quantidade: {{$get_count_geral_comissoes_pagas}}</span>
                                </div>                     
                            </div>
                            <button wire:key="btn4" wire:click.prevent="alternarModalidadeComissao(1)" wire:loading.attr="disabled" class="btn btn-sm btn-outline-primary ml-2 p-1" data-tooltip="Comissões à pagar" data-flow="top">
                                <i class="fas fa-sort-alt fa-fw fa-lg"></i>
                            </button>
                ...
                <div wire:target="search_comissao, qtd_comissao" style="margin-top: 125px; margin-bottom: 125px;"
                            ...
                            <tbody class="t-body">
                                ...
                                @foreach ($get_comissoes as $row_comissao)
                                    ...
                                    <tr class="tr-hover @if($row_comissao->status == 1) row-paga  @endif">
                                        ...
                                        <td class="align-middle font-desc">
                                            @if (empty($row_comissao->user->chave_pix))
                                                <span style="color: #dc2626;" class="text-bold">Não cadastrada</span>
                                            @else
                                                {{$row_comissao->user->chave_pix}}
                                            @endif
                                        </td>

                                        <td class="align-middle font-desc">
                                            @if (empty($row_comissao->user->banco))
                                                <span style="color: #dc2626;" class="text-bold">Não cadastrado</span>
                                            @else
                                                {{$row_comissao->user->banco}}
                                            @endif
                                        </td>
                                        ...
                                        @if ($row_comissao->status == 0)
                                        <td class="align-middle">
                                            <div wire:key="{{$row_comissao->id . 'pag'}}" class="div-btns-actions text-center">

                                                @if (empty($row_comissao->user->chave_pix) or empty($row_comissao->user->banco))
                                                
                                                <button data-tooltip="Solicite os dados restantes para pagar a comissão." data-flow="left" class="btn btn-secondary btn-sm mr-1" disabled>
                                                    <i class="far fa-money-bill-alt fa-fw mr-2"></i>Pagar
                                                </button>

                                                @else

                                                <button wire:click.prevent="confirmationComissao({{$row_comissao->id}})" wire:loading.attr="disabled" wire:target="confirmationComissao({{$row_comissao->id}})" class="btn btn-success btn-sm mr-1">
                                                    <i class="far fa-money-bill-alt fa-fw mr-2"></i>Pagar
                                                </button>
                                        ...
                                        @elseif($row_comissao->status == 1)
                                            <td class="align-middle">
                                                <div wire:key="{{$row_comissao->id . 'ok'}}" class="div-btns-actions text-center">
    ...
    <!-- Modal Confirmação Pagamento de Comissão-->
    <div class="modal fade" data-backdrop="static" data-keyboard="false" id="pay-comissao-confirmation" tabindex="-1"
    aria-labelledby="pay-comissao-confirmationLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog">
            <div class="modal-content modal-custom">
                <div class="modal-header">
                    <h5 class="modal-title px-3 py-3" id="pay-comissao-confirmationLabel">Confirmação de pagamento</h5>
                    <button wire:click.prevent="resetComissao()" wire:loading.attr="disabled" type="button" class="close px-4"
                ...
                <div class="modal-body py-4 px-4">
                    ...
                    <h5 class="modal-confirmation-msg m-0 text-center px-4 my-3">Deseja realmente pagar esta comissão?</h5>

                    <div class="confirmation-msg text-center mb-3">
                        <p class="m-0 mb-3 px-4">
                            Ao clicar em <span class="msg-bold">Confirmar</span>, a comissão será paga para o usuário comissionado.
                            <br>
                            <span class="msg-bold text-uppercase">Atenção:</span> Esta ação é irreversível e a comissão não poderá ser estornada!
                        </p>                      
                    </div>

                </div>
                <div class="modal-footer py-4">
                    <button wire:loading.attr="disabled" wire:click.prevent="resetComissao()" type="button"
                        class="btn btn-cancel" data-dismiss="modal">Cancelar</button>
                    <button wire:click.prevent="payComissao({{$comissao_target}})" wire:loading.attr="disabled" type="button" class="btn btn-send" data-dismiss="modal">
                        Confirmar
                    </button>
                </div>
            </div>
