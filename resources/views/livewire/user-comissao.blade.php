<div>
    ...
    <div class="page-header d-flex flex-row align-items-center mb-2">
        ...
        <button data-toggle="modal" data-target="#como-funciona" class="btn btn-new ml-auto"><i
                class="fad fa-info-circle fa-fw fa-lg mr-2"></i>Como funciona?</button>
    </div>
        ...
        <div class="card">
            ...
            <div class="topo-ico d-flex flex-row align-items-center mb-1">
                ...
                @if ($modalidade_comissao === 1)
                    ...
                    <button wire:key="btn3" wire:click.prevent="alternarModalidadeComissao(0)"
                        wire:loading.attr="disabled" class="btn btn-sm btn-outline-success ml-2 p-1"
                        data-tooltip="Comissões recebidas" data-flow="top">
                ...
                @elseif($modalidade_comissao === 0)
                    ...
                    <button wire:key="btn4" wire:click.prevent="alternarModalidadeComissao(1)"
                        wire:loading.attr="disabled" class="btn btn-sm btn-outline-primary ml-2 p-1"
                        data-tooltip="Comissões à receber" data-flow="top">
                        <i class="fas fa-sort-alt fa-fw fa-lg"></i>
                    </button>
                @endif
                <div class="dropdown" wire:key="drop3">
                    <button style="padding: 5px 5px; color: #666;" class="btn btn-sm btn-light ml-2 rounded-circle"
                        type="button" id="drop_details3" data-toggle="dropdown" aria-expanded="false">
                    ...
                    <div style="padding: 20px !important; width: 400px !important; max-width: 400px !important; min-width: 400px !important;"
                        class="dropdown-menu text-uppercase" aria-labelledby="drop_details3">
            ...
            <div wire:target="qtd_comissao" style="margin-top: 125px; margin-bottom: 125px;" wire:loading
            ...
            <div wire:target="qtd_comissao" wire:loading.remove
                class="card-body px-0 pb-0 table-responsive yampay-scroll">
                            ...
                            @foreach ($get_comissoes as $row_comissao)
                                ...
                                <tr class="tr-hover @if ($row_comissao->status == 1) row-paga @endif">
                                    ...
                                    <td class="align-middle">
                                        ...
                                        <span style="font-size: 11px; background: #3b82f6;"
                                    ...
                                    @elseif($row_comissao->status == 1)
                                        <td class="align-middle">
                                            <div class="div-btns-actions text-center">
                                                ...
                                                @if ($row_comissao->valor == 15.0)
                                                    <button wire:click.prevent="openReceipt({{ $row_comissao->id }})"
                                                        wire:target="openReceipt({{ $row_comissao->id }})"
                                                ...
                                                @else
                                                    <button wire:ignore data-toggle="tooltip" data-placement="top"
                                                        title="Recibo indisponível para comissões recebidas de planos com valor promocional."
                                                        style="opacity: .65; cursor: not-allowed;" type="button"
                ...
                @else
                    <div class="d-flex flex-column align-items-center justify-content-center">
                        ...
                        <div class="d-flex flex-column align-items-center justify-content-center mb-4">
                            ...
                            <button data-toggle="modal" data-target="#como-funciona" class="ml-2 btn btn-nr"><i
                                    class="fad fa-info-circle fa-fw fa-lg mr-2"></i>Como funciona?</button>
                        </div>
                    </div>
    ...
    {{-- MODAL COMO FUNCIONA --}}
    <div class="modal fade" id="como-funciona" tabindex="-1" aria-labelledby="como-funcionaLabel"
        aria-hidden="true">
                ...
                <div class="modal-body py-4 px-4">
                    ...
                    <div style="z-index: 10; position: relative;" class="confirmation-msg text-center">
                        <p style="font-size: 18px;" class="m-0 mb-4 px-4">
                            Convide <span class="msg-bold">um amigo ou empresa</span> para se juntar à plataforma.
                            <br>Assim que seu parceiro efetivar um plano conosco e <span class="msg-bold">realizar o
                                primeiro pagamento</span>, você receberá uma comissão de <span style="font-size: 20px;"
                                class="msg-bold">10%</span> diretamente em sua conta!
                        </p>

                        @php
                            $stripped_name = preg_replace('/\s+/', '%20', auth()->user()->name . ' [Código: SY-' . auth()->user()->id . ']');
                        @endphp

                        <button onclick="copyInvite()" id="btn-invite"
                            data-clipboard-text="Olá!! Estou-lhe convidando a fazer parte da melhor *Plataforma de Gerenciamento Financeiro - {{ config('app.name') }}*! Você não pode ficar fora dessa! 🤑&#10;&#10;🔗 *Meu link de convite:* https://api.whatsapp.com/send?phone={{ config('app.wpp') }}&text=Ol%C3%A1!%20Fui%20indicado%20a%20participar%20da%20Plataforma%20{{ config('app.name') }}%20pelo(a)%20*{{ $stripped_name }}*%2C%20quero%20saber%20mais%20detalhes%2C%20como%20funciona%3F"
                            style="font-size: 18px;" class="copy-invite btn btn-new mb-4">Copiar link de convite <i
                                class="fa-fw fas fa-clone ml-1"></i></button>

                        <h5 style="font-size: 25px;" class="modal-confirmation-msg m-0 text-center px-4 mb-3">Atenção!
                        </h5>

                        <p style="font-size: 18px;" class="m-0 mb-3 px-4">
                            Para começar a <span class="msg-bold">receber pagamentos</span>, você deve obrigatoriamente
                            ter <span class="msg-bold">uma chave PIX</span> e um <span class="msg-bold">banco ou
                                instituição</span> cadastrados em sua conta.
                        </p>

                        <p style="font-size: 18px;" class="m-0 mb-3 px-4">
                            Se você ainda não realizou esses cadastros, <a style="font-size: 18px !important;"
                                class="verify-font" href="{{ route('profile.show') }}">clique aqui</a> para ir até
                            seu perfil e completar a verificação.
                        </p>

                        @if (empty(auth()->user()->chave_pix) or empty(auth()->user()->banco))
                            <span style="color: #dc2626; font-size: 16px;"><i
                                    class="fad fa-times-circle fa-fw mr-2"></i>Você ainda não possui uma chave PIX ou
                                um banco/instituição cadastrados em sua conta.</span>
                        @else
                            <span style="color: #16a34a; font-size: 16px;"><i
                                    class="fad fa-check-circle fa-fw mr-2"></i>Tudo certo! Você já possui uma chave PIX
                                e um banco/instituição cadastrados em sua conta e já pode receber pagamentos!</span>
                        @endif
                        ...
                        <a style="font-size: 18px !important; padding-bottom: 30px;"
                            ...
                            target="_blank" type="button" class="px-4 verify-font">Fale conosco</a><br>
                    </div>

                    <img style="position: absolute; margin-top: -55px; left: -4px; bottom: -20px; z-index: 5;"
                        class="urso-img" src="{{ asset('vendor/adminlte/dist/img/no-results-300.png') }}">
                    ...
                    <div class="confirmation-msg">
                        ...
                        @if (isset($recibo_info))
                                ...
                                <div class="col d-flex flex-column align-items-center">
                                    <div class="div-number-mensalidade ml-auto">
                                        <span style="font-size: 18px; color: #725BC2;">Nº: <span
                                                style="font-weight: 600;">{{ $recibo_info['0']['comission_id'] }}</span></span>
                                    </div>
                            ...
                            <div class="div-row-3 mb-4">
                                <span style="font-size: 17px; color: #444;">
                                    Pagamento recebido por: <span
                                        style="font-weight: 600;">{{ $recibo_info['0']['nome'] }}</span> - Chave PIX:
                            ...
                            <div class="row mb-5">
                                ...
                                <div class="col d-flex flex-column align-items-center">
                                    <div class="div-final-recibo ml-auto">
                                        <span style="font-size: 17px; color: #444;">Patrocínio - MG,
                                            {{ $data_info['0'] }} de {{ $data_info['1'] }} de
                                            {{ $data_info['2'] }}</span>
                                    </div>
                                </div>
